...
class oracleConnexion(nc.noConnexion):
            ...
            if "ssh" in host:
                ...
                if "password" in sshParams:
                    if sshParams["password"] == "":
                        sshParams["password"] = raw_input(
                            "Password de l'usuari {} a {}: ".format(sshParams["username"], host["hostname"]))
                else:
                    sshParams["password"] = raw_input(
                        "Password de l'usuari {} a {}: ".format(sshParams["username"], host["hostname"]))
    ...
    def __insertVectorOutliers(self, nameDataset, conf, repeticio, outliersGTIdx, vectorBlobs, dataInfo):
        for i, id in enumerate(outliersGTIdx):
            self.cursor.callproc("gabd.insertVectorOutliers",
                                 [nameDataset, "-".join([str(x) for x in conf]), repeticio, id, vectorBlobs[i]])
    ...
    def insertExperiment(self, conf, repeticio, method, paramsMethod):
        ...
        idEM = self.cursor.callfunc("GABD.insertExperiment", int,
                                    ["-".join([str(x) for x in conf]), conf[0], conf[1], repeticio, method, paramCode,
                                     paramsMethod.keys(), [float(x) for x in paramsMethod.values()]])
    ...
    def insertResults(self, nameDataset, idExperiment, fpr, tpr, auc, dataInfo):
        ...
        self.cursor.callproc("GABD.insertResultats",
                             [nameDataset, idExperiment, dataInfo["type"], auc, blobFPR, blobTPR])
