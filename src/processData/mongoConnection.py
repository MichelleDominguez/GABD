...
class mongoConnexion(nc.noConnexion):
        ...
        if numServers == 1:
            ...
            if "ssh" in host:
                ...
                sshParams["password"] = raw_input(
                    "Password de l'usuari {} a {}: ".format(sshParams["username"], sshParams["hostname"]))
                ...
                DSN = "mongodb://{}:{}@{}:{}/{}".format(cfg.username, cfg.password, host["hostname"], host["port"],
                                                        cfg.db)
                        ...
                        if res.count() > 0:
                            for row in res:
                                if "label" in row:
                                    ...
                                    label = [nom]
                                collection.update({"Dataset": dataset, "id": row["id"]},
                                                  {"Dataset": dataset, "id": row["id"], "name": row["name"],
                                                   "label": label}, upsert=True)
                        # else:
                        #  label = [nom]
    ...
    def insertExperiment(self, conf, repeticio, method, paramsMethod):
        ...
        if res.count() > 0:
            idEM = res[0]["_id"]
            print "Ja existeix l'experiment"
        else:
            print "No existeix l'experiment"
            res = []
            idEM = res
            collection.insert_one(filter_query)
    ...
    def __loadImageData(self, nameDataset, dataInfo):
        ...
        res = collection.find({"Dataset": nameDataset, "cnn": dataInfo["cnn"], "layer": dataInfo["layers"]},
                              {"id": 1, "features": 1, "label": 1})
